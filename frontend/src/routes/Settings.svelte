...
<div>
  <header class="h-20 flex items-center w-[90%] justify-start mx-auto relative">
    <button class='btn text-warning h-5 w-[12rem] text-2xl bg-base-100 rounded-full' on:click={async () => { await push('/home')}}>
      <svg class="h-full" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M5.82843 6.99955L8.36396 9.53509L6.94975 10.9493L2 5.99955L6.94975 1.0498L8.36396 2.46402L5.82843 4.99955H13C17.4183 4.99955 21 8.58127 21 12.9996C21 17.4178 17.4183 20.9996 13 20.9996H4V18.9996H13C16.3137 18.9996 19 16.3133 19 12.9996C19 9.68584 16.3137 6.99955 13 6.99955H5.82843Z"></path></svg>
      Go Back
    </button>
    <h1 class='text-[3rem] text-center grow mx-auto font-mserrat text-primary'>Settings</h1>
    <div>
      <ThemeSelect size={'16'} />
    </div>
  ...
  <div class='w-[90%] mx-auto flex flex-col items-center mt-10 gap-5'>
    <div class='ml-28'>
      <Profile moniker={userInfo.username[0].toUpperCase()} status={'online'} width={'48'} fontSize={'[5rem]'}/>
    </div>

    <div class='relative ml-28 px-[2.75rem]'>
      {#if !validUsername}
      <p class='pl-2 text-error mb-1 max-w-md'>Username must be atleast 4 characters long</p>
      {/if}
      <input type="text"
      ...
      disabled={udisabled}
      />
      <button class='absolute right-0' on:click={() => {udisabled = !udisabled}}>
    ...
    <div class='relative ml-28 px-[5.5rem]'>
      {#if !validPassword}
      <p class='pl-2 text-error mb-1 max-w-md'>Password must be 8-16 characters long, with atleast one Uppercase, Lowercase and digit</p>
      {/if}
      ...
      disabled={pdisabled}
      />
      <button class='absolute right-10' on:click={() => {viewPassword = !viewPassword}}>
        <kbd class="kbd kbd-3xl">👁️‍🗨️</kbd>
      </button>
      <button class='absolute right-0' on:click={() => {pdisabled = !pdisabled}}>
        <kbd class="kbd kbd-3xl">✎</kbd>
      </button>
    </div>
    <button

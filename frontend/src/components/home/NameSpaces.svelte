...
<div
  class="w-[6.75rem] max-h-screen border-r-4 border-r-neutral mt-5 flex flex-col items-center gap-y-10"
>
  <button
    id="globalNamespace"
    data-ns="/"
    class={currentNameSpace === "/" &&
      "bg-primary rounded-xl text-secondary-content"}
    on:click={(e) => {
      currentNameSpace = document.getElementById("globalNamespace").dataset.ns;
    }}
  >
  ...
  <button
    id="privateNamespace"
    data-ns="/private"
    class={currentNameSpace === "/private" &&
      "bg-primary rounded-xl text-secondary-content"}
    on:click={(e) => {
      currentNameSpace = document.getElementById("privateNamespace").dataset.ns;
    }}
  >
    <div class="avatar">
      <div class="w-16 rounded-xl">
  ...
  <button
    id="notifyNamespace"
    data-ns="/notify"
    class={currentNameSpace === "/notify" &&
      "bg-primary rounded-xl text-secondary-content"}
    on:click={(e) => {
      currentNameSpace = document.getElementById("notifyNamespace").dataset.ns;
    }}
  >
    <div class="avatar">
      <div class="w-16 rounded-xl">

...
{#if message?.system || system}
  <div class="chat chat-end">
    ...
    <pre class="chat-bubble chat-bubble-warning">{message.translation
        ? message.translation
        : message.text}</pre>
  </div>
{:else if message.username === username}
  <div class="chat chat-start">
    <div class="chat-image avatar">
      <Profile
        moniker={message.username[0].toUpperCase()}
        status={"online"}
        width={"10"}
        fontSize={"xl"}
      />
...
{:else}
  <div class="chat chat-end">
    ...
    <pre class="chat-bubble chat-bubble-primary">{message.translation
        ? message.translation
        : message.text}</pre>
  </div>
{/if}
